...
def has_cycles(edge)
    ...
    @minimum_spanning_tree.each { |x| x[:explore] = false }
    cycle_between(node_one, node_two, @minimum_spanning_tree.dup)
end

def cycle_between(one, two, edges)
    adjacent_edges = edge.select { |edge| edge[:to] == one || edge[:from] == one}
    return false if adjacent_edges.empty?
    adjacent_edges.reject {|edge| edge[:explore] }.each do |edge|
        edge[:explore] = true
...
edges = file.drop(1).map { |x| x.gsub(/\n/,"").split("").map(&:to_i)}.map { |one, two, weight| { :from => one, :to => two, :weight => weight}}.sort_by{|x| x[weight]}
